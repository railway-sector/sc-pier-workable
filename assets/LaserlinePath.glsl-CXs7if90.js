import{kk as Pe,mA as be,pG as we,EG as f,pv as w,pB as De,po as B,In as r,FC as U,Gm as Y,Bn as Ve,ps as C,oq as ee,iN as R,jr as H,pH as xe,BO as te,EN as ie,pq as M,bi as m,Ap as v,N2 as ne,zC as se,yu as o,AD as ae,hy as Ee,v0 as ye,fe as Le,ls as re,yK as le,Kz as Se,Iv as Ce,tN as Me,It as Ae,pw as $e,zR as F,oi as Re,rB as oe,Io as ce,jb as he,lT as Te,DE as G,KA as q,j8 as c,lj as z,Ht as de,fK as pe,og as Oe,J9 as O,iS as Ie,EJ as qe,DR as je,km as Ue,u7 as V,kl as ze,rE as k,ES as Ne,DT as We,CK as T,Bp as Be,xv as He,KL as Fe,eS as Ge,BQ as ke,kK as Ke,ta as Je,jd as A,l7 as Qe,Hg as K}from"./index-DGYnUy-C.js";import{t as Xe}from"./VisualElement-D0Q8utWg.js";function ue(t,e){const n=t.fragment;n.include(Pe),t.include(be),n.include(we),n.uniforms.add(new f("globalAlpha",i=>i.globalAlpha),new w("glowColor",i=>i.glowColor),new f("glowWidth",(i,s)=>i.glowWidth*s.camera.pixelRatio),new f("glowFalloff",i=>i.glowFalloff),new w("innerColor",i=>i.innerColor),new f("innerWidth",(i,s)=>i.innerWidth*s.camera.pixelRatio),new De("depthMap",i=>i.depth?.attachment),new B("normalMap",i=>i.normals)),n.code.add(r`vec4 premultipliedColor(vec3 rgb, float alpha) {
...
}`)}const N=U(6);function fe(t){const e=new Y;e.include(Ve),e.include(ue,t);const n=e.fragment;if(t.lineVerticalPlaneEnabled||t.heightManifoldEnabled)if(n.uniforms.add(new f("maxPixelDistance",(i,s)=>t.heightManifoldEnabled?2*s.camera.computeScreenPixelSizeAt(i.heightManifoldTarget):2*s.camera.computeScreenPixelSizeAt(i.lineVerticalPlaneSegment.origin))),n.code.add(r`float planeDistancePixels(vec4 plane, vec3 pos) {
float dist = dot(plane.xyz, pos) + plane.w;
float width = fwidth(dist);
dist /= min(width, maxPixelDistance);
return abs(dist);
}`),t.spherical){const i=(a,l,p)=>m(a,l.heightManifoldTarget,p.camera.viewMatrix),s=(a,l)=>m(a,[0,0,0],l.camera.viewMatrix);n.uniforms.add(new C("heightManifoldOrigin",(a,l)=>(i(u,a,l),s(D,l),ee(D,D,u),R(d,D),d[3]=H(D),d)),new xe("globalOrigin",a=>s(u,a)),new f("cosSphericalAngleThreshold",(a,l)=>1-Math.max(2,te(l.camera.eye,a.heightManifoldTarget)*l.camera.perRenderPixelRatio)/H(a.heightManifoldTarget))),n.code.add(r`float globeDistancePixels(float posInGlobalOriginLength) {
...
vec4 color = vec4(0.0);`),t.heightManifoldEnabled){n.uniforms.add(new M("angleCutoff",s=>$(s)),new C("heightPlane",(s,a)=>ge(s.heightManifoldTarget,s.renderCoordsHelper.worldUpAtPosition(s.heightManifoldTarget,u),a.camera.viewMatrix)));const i=t.spherical?r`normalize(globalOrigin - pos)`:r`heightPlane.xyz`;n.main.add(r`
      ...
      color = max(color, heightManifoldColor * heightManifoldAlpha);`)}return t.pointDistanceEnabled&&(n.uniforms.add(new M("angleCutoff",i=>$(i)),new C("pointDistanceSphere",(i,s)=>Ze(i,s))),n.main.add(r`float pointDistanceSphereDistance = sphereDistancePixels(pointDistanceSphere, pos);
